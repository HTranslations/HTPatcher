...
func PatchSystem(data []byte, patchInfo PatchInfo) ([]byte, error) {
	...
	for i := range system.ArmorTypes {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.ArmorTypes[i])]; ok {
			system.ArmorTypes[i] = translation
		}
	}

	for i := range system.Elements {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Elements[i])]; ok {
			system.Elements[i] = translation
		}
	}

	for i := range system.EquipTypes {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.EquipTypes[i])]; ok {
			system.EquipTypes[i] = translation
		}
	}

	for i := range system.SkillTypes {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.SkillTypes[i])]; ok {
			system.SkillTypes[i] = translation
		}
	}

	for i := range system.WeaponTypes {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.WeaponTypes[i])]; ok {
			system.WeaponTypes[i] = translation
		}
	}

	for i := range system.Switches {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Switches[i])]; ok {
			system.Switches[i] = translation
		}
	}

	for i := range system.Variables {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Variables[i])]; ok {
			system.Variables[i] = translation
		}
	}

	for i := range system.Terms.Basic {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Basic[i])]; ok {
	...
	for i := range system.Terms.Commands {
		if system.Terms.Commands[i] != nil {
			if translation, ok := patchInfo.Dictionary[GetTranslationKey(*system.Terms.Commands[i])]; ok {
	...
	for i := range system.Terms.Params {
		if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Params[i])]; ok {
			system.Terms.Params[i] = translation
		}
	}

	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.AlwaysDash)]; ok {
		system.Terms.Messages.AlwaysDash = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.CommandRemember)]; ok {
		system.Terms.Messages.CommandRemember = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.TouchUI)]; ok {
		system.Terms.Messages.TouchUI = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.BgmVolume)]; ok {
		system.Terms.Messages.BgmVolume = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.BgsVolume)]; ok {
		system.Terms.Messages.BgsVolume = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.MeVolume)]; ok {
		system.Terms.Messages.MeVolume = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.SeVolume)]; ok {
		system.Terms.Messages.SeVolume = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Possession)]; ok {
		system.Terms.Messages.Possession = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ExpTotal)]; ok {
		system.Terms.Messages.ExpTotal = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ExpNext)]; ok {
		system.Terms.Messages.ExpNext = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.SaveMessage)]; ok {
		system.Terms.Messages.SaveMessage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.LoadMessage)]; ok {
		system.Terms.Messages.LoadMessage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.File)]; ok {
		system.Terms.Messages.File = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Autosave)]; ok {
		system.Terms.Messages.Autosave = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.PartyName)]; ok {
		system.Terms.Messages.PartyName = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Emerge)]; ok {
		system.Terms.Messages.Emerge = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Preemptive)]; ok {
		system.Terms.Messages.Preemptive = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Surprise)]; ok {
		system.Terms.Messages.Surprise = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EscapeStart)]; ok {
		system.Terms.Messages.EscapeStart = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EscapeFailure)]; ok {
		system.Terms.Messages.EscapeFailure = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Victory)]; ok {
		system.Terms.Messages.Victory = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Defeat)]; ok {
		system.Terms.Messages.Defeat = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ObtainExp)]; ok {
		system.Terms.Messages.ObtainExp = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ObtainGold)]; ok {
		system.Terms.Messages.ObtainGold = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ObtainItem)]; ok {
		system.Terms.Messages.ObtainItem = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.LevelUp)]; ok {
		system.Terms.Messages.LevelUp = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ObtainSkill)]; ok {
		system.Terms.Messages.ObtainSkill = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.UseItem)]; ok {
		system.Terms.Messages.UseItem = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.CriticalToEnemy)]; ok {
		system.Terms.Messages.CriticalToEnemy = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.CriticalToActor)]; ok {
		system.Terms.Messages.CriticalToActor = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorDamage)]; ok {
		system.Terms.Messages.ActorDamage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorRecovery)]; ok {
		system.Terms.Messages.ActorRecovery = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorGain)]; ok {
		system.Terms.Messages.ActorGain = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorLoss)]; ok {
		system.Terms.Messages.ActorLoss = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorDrain)]; ok {
		system.Terms.Messages.ActorDrain = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorNoDamage)]; ok {
		system.Terms.Messages.ActorNoDamage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActorNoHit)]; ok {
		system.Terms.Messages.ActorNoHit = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyDamage)]; ok {
		system.Terms.Messages.EnemyDamage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyRecovery)]; ok {
		system.Terms.Messages.EnemyRecovery = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyGain)]; ok {
		system.Terms.Messages.EnemyGain = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyLoss)]; ok {
		system.Terms.Messages.EnemyLoss = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyDrain)]; ok {
		system.Terms.Messages.EnemyDrain = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyNoDamage)]; ok {
		system.Terms.Messages.EnemyNoDamage = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.EnemyNoHit)]; ok {
		system.Terms.Messages.EnemyNoHit = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Evasion)]; ok {
		system.Terms.Messages.Evasion = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.MagicEvasion)]; ok {
		system.Terms.Messages.MagicEvasion = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.MagicReflection)]; ok {
		system.Terms.Messages.MagicReflection = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.CounterAttack)]; ok {
		system.Terms.Messages.CounterAttack = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.Substitute)]; ok {
		system.Terms.Messages.Substitute = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.BuffAdd)]; ok {
		system.Terms.Messages.BuffAdd = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.DebuffAdd)]; ok {
		system.Terms.Messages.DebuffAdd = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.BuffRemove)]; ok {
		system.Terms.Messages.BuffRemove = translation
	}
	if translation, ok := patchInfo.Dictionary[GetTranslationKey(system.Terms.Messages.ActionFailure)]; ok {
