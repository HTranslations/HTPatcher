...
export namespace util {
	...
	export class WrapperMeta {
	    UID: string;
	    BID: string;
	
	    static createFrom(source: any = {}) {
	        return new WrapperMeta(source);
	    }
	...
	export class WrapperCipher {
	    Meta: Record<string, WrapperMeta>;
	
	    static createFrom(source: any = {}) {
	        return new WrapperCipher(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.Meta = this.convertValues(source["Meta"], WrapperMeta, true);
	    }
